...
Если не учитывать результат `allPaths`, стек рекурсии и текущий путь требуют до $$O(N)$$ памяти в вырожденном дереве. В сбалансированном бинарном дереве может быть до $$N/2$$ листьев, а длина каждого сохранённого пути не превышает $$O(\log N)$$. Поэтому в исходной оценке для результата и общей пространственной сложности указано $$O(N \log N)$$.
